...
def roll_up_tmx_payer( day_start,  n_Days):
    ...
    for iDay in range(nDays):
        ...
        print "rolling up payer threatmetrix signals for: ",str(day)
...
def roll_up_tmx_payer_helper(arg):
    roll_up_tmx_payer(arg,1) # always 1 to deal with one day, multiple days are dealt by pool
...
for i in range(nDays):
    ...
    if os.path.exists(work_dir + "threatmetrix_payer_flat_"+str(dayToProcess)+".csv.gz"):
        print "threatmetrix_payer_flat_"+str(dayToProcess)+".csv.gz"," already exits, skipping ..."
    else:
        print "threatmetrix_payer_flat_"+str(dayToProcess)+".csv.gz"," rollup will be processed"
