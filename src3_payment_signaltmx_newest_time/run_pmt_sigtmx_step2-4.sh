...
if [ $opt == 1 ]; then
	...
	python ${code_dir}step2_model_data_preprocessing/step2a_data_concat_daily_files.py $year $month $day
	python ${code_dir}step2_model_data_preprocessing/step2b_model_data_split_downsample.py $data_dir
	python ${code_dir}step3_model_data_prep_signal_tmx/step3a_model_data_rc_tmxrc_ind_creation_main.py $data_dir
	python ${code_dir}step3_model_data_prep_signal_tmx/step3b_model_data_feature_creation_main.py $data_dir
	python ${code_dir}step3_model_data_prep_signal_tmx/step3c_support_impute_woe_mapping_calculation.py $data_dir
	python ${code_dir}step3_model_data_prep_signal_tmx/step3c_model_data_impute_woe_assigin_main.py $data_dir
	
	python ${code_dir}step4_model_train/step4_model_train_signal_tmx_v2.py $data_dir $result_dir
